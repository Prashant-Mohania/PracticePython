...
class Main(MDApp):
	...
	def select_res(self , instance):
		try:
			...
			self.bottomsheet.add_item("Resolution                 Type                 Size    ", lambda x: None)

            try:
                self.k128 = self.v_yt.streams.filter(only_audio=True).first()
                bottom_res.add_item(
                    "128K                    mp3                        size = {self.k128.filesize/1000000:.3f} MB", lambda x: self.Download(self.k128))
			...
			try:
				...
				self.bottomsheet.add_item(f"144p                 mp4                 {self.r144[0].filesize:.2f} MB", lambda x: self.Download(self.r144))
				
			except:
				self.bottomsheet.add_item(f"144p                 mp4                 Unavailable", lambda x: sys.exit())
				
			try:
				self.r240 = self.v_yt.streams.filter(file_extension='mp4', res='240p', progressive=True).all()
				self.bottomsheet.add_item(f"240p                 mp4                 {self.r240[0].filesize:.2f} MB", lambda x: self.Download(self.r240))
				
			except:
				self.bottomsheet.add_item(f"240p                 mp4                 Unavailable", lambda x: sys.exit())

			try:
				self.r360 = self.v_yt.streams.filter(file_extension='mp4', res='360p', progressive=True).all()
				self.bottomsheet.add_item(f"360p                 mp4                 {self.r360[0].filesize/1000000:.2f} MB", lambda x: self.Download(self.r360))
				
			except:
				self.bottomsheet.add_item(f"360p                 mp4                 Unavailable", lambda x: sys.exit())

			try:
				self.r480 = self.v_yt.streams.filter(file_extension='mp4', res='480p', progressive=True).all()
				self.bottomsheet.add_item(f"480p                 mp4                 {self.r480[0].filesize:.2f} MB", lambda x: self.Download(self.r480))
				
			except:
				self.bottomsheet.add_item(f"480p                 mp4                 Unavailable", lambda x: sys.exit())

			try:
				self.r720 = self.v_yt.streams.filter(file_extension='mp4', res='720p', progressive=True).all()
				self.bottomsheet.add_item(f"720p                 mp4                 {self.r720[0].filesize:.2f} MB", lambda x: self.Download(self.r720))
				
			except:
				self.bottomsheet.add_item(f"720p                 mp4                 Unavailable", lambda x: sys.exit())
